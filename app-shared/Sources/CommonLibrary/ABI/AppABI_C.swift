...
public func __psp_app_init(
    ...
    // A NULL config_path selects the default location, matching the Apple app;
    // a non-NULL value overrides it.
    let configFilePath = ConfigFileApplier.resolvedConfigPath(
        args.pointee.config_path.map { String(cString: $0) }
    )
