...
android_resources="$android_dir/app/src/main/res"
...
def verify_locale!(locale, base_keys, locale_keys)
  required_base_keys = base_keys.reject { |key| key.start_with?("android.") }
  missing = required_base_keys - locale_keys
...
def verify_placeholders!(locale, base_entries, entries_by_key)
  ...
  base_entries.each do |key, base_value|
    next if key.start_with?("android.") && !entries_by_key.key?(key)
...
lproj_dirs.each do |dirname|
  ...
  xml << "<resources>\n"
  xml << "    <!-- Generated by app-android/scripts/import-apple-strings.sh. Do not edit directly. -->\n"

  base_keys.each do |key|
    next unless entries_by_key.key?(key)
...
puts "Imported Apple translations into Android string resources."
RUBY
