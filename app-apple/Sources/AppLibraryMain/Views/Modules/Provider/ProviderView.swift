...
private extension ProviderView {
    ...
    var resolvedModule: Module? {
        do {
            ...
            pspLog(.core, .debug, "Unable to resolve provider module: \(error)")
            return nil
        }
    }
    ...
    func refreshIndex() async {
        do {
            ...
            pspLog(.core, .error, "Unable to fetch index: \(error)")
        }
    }
    ...
    func refreshInfrastructure(for newProviderModule: ProviderModule) async {
        do {
            ...
            pspLog(.core, .error, "Unable to refresh infrastructure: \(error)")
        }
    }
    ...
    func loadSupportedPresets(for server: ProviderServer) async {
        ...
        do {
            ...
            pspLog(.core, .error, "Unable to fetch presets for current server: \(error)")
            availablePresets = []
        }
    }
    ...
    func loadPreferences(for newProviderId: ProviderID?) {
        if let newProviderId {
            do {
                pspLog(.core, .debug, "Load preferences for provider \(newProviderId)")
                ...
                pspLog(.core, .error, "Unable to load preferences for provider \(newProviderId): \(error)")
    ...
    func savePreferences() {
        ...
        do {
            pspLog(.core, .debug, "Save preferences for provider \(providerId)")
            try providerPreferences.save()
        } catch {
            pspLog(.core, .error, "Unable to save preferences for provider \(providerId): \(error)")
        }
    }
}
